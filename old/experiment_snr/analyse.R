...
write.csv(parameters, file="./experiment_snr/parameters.csv", row.names=F)
# ------------------------------------------------------------------------------
...
prepare_results = function(result){
  ...
  B1 = fit$vc_path[2,,]
  t0 = fit$scaled_time
  b1true = LSVCMM::generate_synthetic_data()$true_values$b1
  # compute ICs
  AIC = -2*llk + 2*df
  AICs = -2*llk + 2*dfs
  BIC = -2*llk + df_logn
...
icresultswithpars = icresults %>% left_join(parameters, by="job.id")
write.csv(icresults, file="./experiment_snr/icresults.csv", row.names=F)
# ------------------------------------------------------------------------------
